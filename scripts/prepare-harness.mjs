...
if (run('git', ['status', '--porcelain', '--untracked-files=no'], { stdio: 'pipe' })) {
  throw new Error('Host source must be unmodified; choose a new empty directory')
}
console.log(`Prepared stock Harness ${manifest.version} at ${target}`)
if (values.build) {
  ...
  if (run('git', ['status', '--porcelain', '--untracked-files=no'], { stdio: 'pipe' })) throw new Error('Build changed tracked Host files')
